...
RasterizeGaussiansWithDepthCUDA(
	const torch::Tensor& background,
	const torch::Tensor& means3D,
    const torch::Tensor& colors,
    const torch::Tensor& opacity,
	...
	const bool prefiltered)
{
  ...
  if(P != 0)
  {
	  ...
	  rendered = CudaRasterizer::Rasterizer::forward_with_depth(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
	...
	const bool prefiltered)
{
  ...
  torch::Tensor out_color = torch::full({P, NUM_CHANNELS}, 0.0, float_opts);
  torch::Tensor radii = torch::full({P}, 0, means3D.options().dtype(torch::kInt32));
  torch::Tensor points_image = torch::full({P, 2}, 0, means3D.options().dtype(torch::kFloat32));
  ...
  if(P != 0)
  {
	  ...
	  CudaRasterizer::Rasterizer::project2_image(
	    geomFunc,
		binningFunc,
		imgFunc,
	    P, degree, M,
		...
		prefiltered,
		out_color.contiguous().data_ptr<float>(),
        points_image.contiguous().data_ptr<float>(),
		radii.contiguous().data_ptr<int>());
  }
  return std::make_tuple(points_image, radii, out_color);
}
