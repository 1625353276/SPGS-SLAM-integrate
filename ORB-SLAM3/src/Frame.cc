...
Frame::Frame(const Frame &frame)
    :mpcpi(frame.mpcpi),mpSPvocabulary(frame.mpSPvocabulary),mpORBvocabulary(frame.mpORBvocabulary), mpORBextractorLeft(frame.mpORBextractorLeft), mpORBextractorRight(frame.mpORBextractorRight),
...
Frame::Frame(const cv::Mat &imGray, const cv::Mat &imRGB, const double &timeStamp, ORBextractor* extractor,ORBVocabulary* voc, GeometricCamera* pCamera, cv::Mat &distCoef, const float &bf, const float &thDepth, const cv::Mat &undistorted_img, Frame* pPrevF, const IMU::Calib &ImuCalib)
    :mpcpi(NULL),mpORBvocabulary(voc),mpORBextractorLeft(extractor),mpORBextractorRight(static_cast<ORBextractor*>(NULL)),
...
void Frame::ComputeBoW3()
{
    ...
    if(mBow3Vec.empty())
    {
        ...
        mpSPvocabulary->transform(vCurrentDesc,mBow3Vec,mFeat3Vec,0);
    }
}
...
{
    mvuRight = vector<float>(N,-1);//变量初始化
    mvDepth = vector<float>(N,-1);

    for(int i=0; i<N; i++)//遍历所有特征点
    {
        const cv::KeyPoint &kp = mvKeys[i];//去畸变之前特征点
        const cv::KeyPoint &kpU = mvKeysUn[i];//去畸变特征点

        const float &v = kp.pt.y;
        const float &u = kp.pt.x;//特征点坐标

        const float d = imDepth.at<float>(v,u);//坐标对应深度图中的深度

        if(d>0)
        {
            mvDepth[i] = d;//存储深度
            mvuRight[i] = kpU.pt.x-mbf/d;//存储
        }
    }
}
