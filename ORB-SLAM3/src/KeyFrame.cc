...
KeyFrame::KeyFrame(Frame &F, Map *pMap, KeyFrameDatabase *pKFDB):
    ...
    mpCamera(F.mpCamera), mpCamera2(F.mpCamera2), mpSPvocabulary(F.mpSPvocabulary),
    ...
    mvKeysRight(F.mvKeysRight), NLeft(F.Nleft), NRight(F.Nright), mTrl(F.GetRelativePoseTrl()), mnNumberOfOpt(0), mbHasVelocity(false)
{
    ...
    this->undistortedRGB = F.undistortedRGB;
}
    ...
    std::vector<float> &pointsLocal)
{
    ...
    for(int i = 0; i < N; ++i)
    {
        pixelsUndist.push_back(mvKeysUn[i].pt.x);//u
        pixelsUndist.push_back(mvKeysUn[i].pt.y);//v
        if(mvpMapPoints[i])
        {
