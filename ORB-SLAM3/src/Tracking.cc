...
Tracking::Tracking(System *pSys, SPVocabulary* pVoc, FrameDrawer *pFrameDrawer, MapDrawer *pMapDrawer, Atlas *pAtlas, KeyFrameDatabase* pKFDB, const string &strSettingPath, const int sensor, Settings* settings, const string &_nameSeq):
...
Sophus::SE3f Tracking::GrabImageStereo(const cv::Mat &imRectLeft, const cv::Mat &imRectRight, const double &timestamp, string filename)
{
    ...
    if (mSensor == System::STEREO && !mpCamera2)
        mCurrentFrame = Frame(mImGray,imGrayRight,mImRGB,imRightRGB,timestamp,mpExtractorLeft,mpExtractorRight,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera);
        ...
        mCurrentFrame = Frame(mImGray,imGrayRight,mImRGB,imRightRGB,timestamp,mpExtractorLeft,mpExtractorRight,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera,&mLastFrame,*mpImuCalib);
    else if(mSensor == System::IMU_STEREO && mpCamera2)
...
Sophus::SE3f Tracking::GrabImageRGBD(const cv::Mat &imRGB, const cv::Mat &imD, const double &timestamp, string filename, const cv::Mat &undistorted_img)
{
    mImGray = imRGB;
    cv::Mat imDepth = imD;
    ...
    //undistorted_img
    cv::Mat undistorted_RGB = undistorted_img;
    undistorted_img.copyTo(undistorted_RGB);
    if (undistorted_RGB.type() == CV_8UC3)
        undistorted_RGB.convertTo(undistorted_RGB, CV_32FC3, 1.0 / 255.0);
    else if (undistorted_RGB.type() == CV_16UC3)
        undistorted_RGB.convertTo(undistorted_RGB, CV_32FC3, 1.0 / 65535.0);
    else if (undistorted_RGB.type() == CV_16FC3 || undistorted_RGB.type() == CV_64FC3)
        undistorted_RGB.convertTo(undistorted_RGB, CV_32FC3, 1.0);

    if((fabs(mDepthMapFactor-1.0f)>1e-5) || imDepth.type()!=CV_32F)
        imDepth.convertTo(imDepth,CV_32F,mDepthMapFactor);

    if (mSensor == System::RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mImRGB,timestamp,mpExtractorLeft,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera, undistorted_RGB);
    else if(mSensor == System::IMU_RGBD)
        mCurrentFrame = Frame(mImGray,imDepth,mImRGB,timestamp,mpExtractorLeft,mpSPVocabulary,mK,mDistCoef,mbf,mThDepth,mpCamera, undistorted_RGB, &mLastFrame,*mpImuCalib);
    //mCurrentFrame后续在track中会用到
...
Sophus::SE3f Tracking::GrabImageMonocular(const cv::Mat &im, const double &timestamp, string filename, const cv::Mat &undistorted_img)
{
    mImGray = im;
    if (mImGray.channels() == 3)
    {
    ...
    if (mSensor == System::MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET ||(lastID - initID) < mMaxFrames)
            mCurrentFrame = Frame(mImGray,mImRGB,timestamp,mpIniExtractor,mpSPVocabulary,mpCamera, mDistCoef, mbf,mThDepth, undistorted_RGB);
        else
            mCurrentFrame = Frame(mImGray,mImRGB,timestamp,mpExtractorLeft,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth, undistorted_RGB);
    }
    else if(mSensor == System::IMU_MONOCULAR)
    {
        if(mState==NOT_INITIALIZED || mState==NO_IMAGES_YET)
        {
            mCurrentFrame = Frame(mImGray,mImRGB,timestamp,mpIniExtractor,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth, undistorted_RGB, &mLastFrame,*mpImuCalib);
        }
        else
            mCurrentFrame = Frame(mImGray,mImRGB,timestamp,mpExtractorLeft,mpSPVocabulary,mpCamera,mDistCoef,mbf,mThDepth, undistorted_RGB, &mLastFrame,*mpImuCalib);
    }
...
void Tracking::Track()
{
    ...
    else
    {
        ...
        if(!mbOnlyTracking)
        {
            if(bOK)
            {
                bOK = TrackLocalMap();//这部分主要就是这个函数,位姿跟踪完成后,进入局部地图构建
...
//                 // std::cout << "pushMappingOperation. " << mCurrentFrame.mnId << " "<< vpKFs.back()->frameID << std::endl;
//                 if(mCurrentFrame.mnId - vpKFs.back()->frameID > 10)
//                 {
// // std::cout << "opr.addFrame " << mCurrentFrame.undistortedRGB.size() << std::endl;
//                     MappingOperation opr(MappingOperation::OprType::Tracking);
//                     opr.addFrame(&mCurrentFrame);
...
void Tracking::MonocularInitialization()
{
    ...
    else
    {
        ...
        int nmatches = mspmatcher.SearchForInitialization(mInitialFrame,mCurrentFrame,mvbPrevMatched,mvIniMatches);
...
bool Tracking::TrackWithMotionModel()
{
    ...
    int nmatches = mspmatcher.SearchBySP(mCurrentFrame,mLastFrame);
    ...
    if(nmatches<20)
    {
        ...
        nmatches = mspmatcher.SearchBySP(mCurrentFrame,mLastFrame);
...
bool Tracking::Relocalization()
{
    ...
    for(int i=0; i<nKFs; i++)
    {
        ...
        {
            int nmatches = mspmatcher.SearchByBoWSP(pKF,mCurrentFrame,vvpMapPointMatches[i]);
            if(nmatches<15)
            {
                vbDiscarded[i] = true;
                continue;
    ...
    while(nCandidates>0 && !bMatch)
    {
        for(int i=0; i<nKFs; i++)
        {
            ...
            if(bTcw)
            {
                ...
                if(nGood<50)
                {
                    int nadditional = mspmatcher.SearchByProjection(mCurrentFrame,vpCandidateKFs[i],sFound,10,100);
                    ...
                    if(nadditional+nGood>=50)
                    {
                        ...
                        if(nGood>30 && nGood<50)
                        {
                            ...
                            nadditional = mspmatcher.SearchByProjection(mCurrentFrame,vpCandidateKFs[i],sFound,3,64);
