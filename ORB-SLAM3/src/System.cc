...
Sophus::SE3f System::TrackStereo(const cv::Mat &imLeft, const cv::Mat &imRight, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename)
{
    ...
    if(settings_ && settings_->needToRectify()){ //针孔相机双目需要这个步骤
        ...
        // std::cout << "remap img" << M1l << M2l << std::endl;
    }
                               ...
                               const cv::Mat &undistorted_img)
{
    ...
    cv::Mat undistorted_imgToFeed = preprocessImage(undistorted_img);
    if(settings_ && settings_->needToResize()){
        cv::Mat resizedIm;
        cv::resize(im,resizedIm,settings_->newImSize());
        imToFeed = resizedIm;

        cv::resize(depthmap,imDepthToFeed,settings_->newImSize());//RGBd
    }
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageRGBD(imToFeed,imDepthToFeed,timestamp,filename, undistorted_imgToFeed);//depth input
...
Sophus::SE3f System::TrackMonocular(const cv::Mat &im, const double &timestamp, const vector<IMU::Point>& vImuMeas, string filename, const cv::Mat &undistorted_img)
{
    ...
    Sophus::SE3f Tcw = mpTracker->GrabImageMonocular(imToFeed,timestamp,filename, undistorted_imgToFeed);
...
vector<KeyFrame*> System::SaveTrajectoryTUM(const string &filename)
{
    ...
    f.close();
    return std::vector<KeyFrame*>(mpTracker->mlpReferences.begin(),mpTracker->mlpReferences.end());
...
vector<KeyFrame*> System::GetAllFrames()
{
    return vector<KeyFrame*>(mpTracker->mlpReferences.begin(),mpTracker->mlpReferences.end());
}

vector<Sophus::SE3f> System::GetAllRelativeFramePoses()
{
    return vector<Sophus::SE3f>(mpTracker->mlRelativeFramePoses.begin(),
        mpTracker->mlRelativeFramePoses.end());
}
