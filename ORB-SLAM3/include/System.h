...
public:
    ...
    Sophus::SE3f TrackRGBD(const cv::Mat &im, const cv::Mat &depthmap, const double &timestamp, const vector<IMU::Point> &vImuMeas = vector<IMU::Point>(), string filename = "",
                           const cv::Mat &undistorted_img = cv::Mat());
    ...
    vector<KeyFrame*> SaveTrajectoryTUM(const string &filename);
...
public:
    ...
    vector<KeyFrame *> GetAllFrames();
    vector<Sophus::SE3f> GetAllRelativeFramePoses();
