...
private:
    MappingOperation(
        ...
        : mvAssociatedKeyFrames(std::move(opr.mvAssociatedKeyFrames)),
          ...
          mfScale(opr.mfScale),
          mT(opr.mT)
        //   mvAssociatedFrames(std::move(opr.mvAssociatedFrames))
    {}
...
//     {
//         std::unique_lock<std::mutex> lock(mMutexKeyFrames);
// // std::cout << "addFrame " << pKF->undistortedRGB.size() << std::endl;
//         mvAssociatedFrames.emplace_back(
//             std::make_tuple(
//                 pKF->mnId+1000,
//                 pKF->mpCamera->GetId(),
//                 pKF->GetPose(),
...
//     }
    ...
    //     double,
    //     cv::Mat/*image*/  >>&
    // associatedFrames() { return mvAssociatedFrames; }
...
protected:
    ...
    //     unsigned long,
    //     double,
    //     cv::Mat/*image*/  >> mvAssociatedFrames;
