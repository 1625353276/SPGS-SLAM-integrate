...
int main(int argc, char **argv)
{
    ...
    example_utils::LoadTrajectoryInt((output_dir / "CameraTrajectory_TUM_bf.txt").string(), pGausMapper->pose_);
    ...
    {        
        ...
        for (idx = 0; idx < Traj.size(); idx++)
        {
            ...
            try {
                ...
                int img_name = example_utils::extractFrameNumber(vstrImageFilenamesRGB[int(Traj[idx][0])]);
                // std::cout << "img_name " << img_name << " and idx " << Traj[idx][0] << std::endl;
                if (img_name != int(Traj[idx][0]))
                {
                    std::cout << "mismatch img_name " << img_name << " and idx " << Traj[idx][0] << std::endl;
                }
                cv::Mat imgRGB = cv::imread(vstrImageFilenamesRGB[int(Traj[idx][0])], cv::IMREAD_UNCHANGED);
            ...
            //compare pose
            if (!vec_kfID.empty()){
            if (int(Traj[idx][0]) == vec_kfID.front())
            {
            ...
            if (int(Traj[idx][0]) != vec_kfID.front())
            {
                ...
                out_dssim   << Traj[idx][0] << " " << std::fixed << std::setprecision(10) << dssim   << std::endl;
                out_psnr_gs << Traj[idx][0] << " " << std::fixed << std::setprecision(10) << psnr_gs << std::endl;
            ...
            if (int(Traj[idx][0]) == vec_kfID.front())
            {
                ...
                out_psnr_compare << Traj[idx][0] << " " << vec_kfID.front() << " " << std::fixed << std::setprecision(10) << psnr_gs << std::endl;
                vec_kfID.pop_front();
