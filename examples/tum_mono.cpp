...
int main(int argc, char **argv)
{
    ...
    for (int ni = 0; ni < nImages; ni++)
    {
        ...
        if (has_undistorted_image)
        {
            ...
            // std::cout << "load():  " << undistorted_img.size[0]  << undistorted_img.size[1] << std::endl; 
        }
        else
            undistorted_img = im;
        ...
        {
            std::cerr << std::endl << "Failed to load undistorted image at: "
                     << std::endl;
            return 1;
        }
        ...
        pSLAM->TrackMonocular(im, tframe, std::vector<ORB_SLAM3::IMU::Point>(), vstrImageFilenamesRGB[ni], undistorted_img);
    ...
    pGausMapper->pSLAM_->SaveTrajectoryTUM((output_dir / "CameraTrajectory_TUM_bf.txt").string());
    ...
    pGausMapper->vTimestamps = &vTimestamps;
    ...
    {        
        ...
        std::deque<unsigned long> vec_kfid(nkfs, 0);
        std::deque<double> vec_kfts(nkfs, 0);
        for (std::size_t i = 0; i < nkfs; ++i) {
            ...
            out_kf << std::fixed  << setprecision(6) << (*kfit).second->TimeStamp << " " << setprecision(9)
        ...
        while(Traj[0][0] > vTimestamps[vec_kfID.front()])
        {
            vec_kfID.pop_front();
        ...
        std::cout << setprecision(6) << Traj[idx][0] << "|" << vTimestamps[vec_kfID.front()] << std::endl;
        ...
        for (idx = 0; idx < Traj.size(); idx++)
        {
            ...
            try {
                if(has_undistorted_image)
                {
                    ...
                    // std::cout << setprecision(6) << Traj[idx][0] << "|" << img_name << std::endl;
            ...
            if (new_kf->set_pose_ && new_kf->set_camera_) {
                new_kf->world_view_transform_ = tensor_utils::EigenMatrix2TorchTensor(
                    new_kf->getWorld2View2(new_kf->trans_, new_kf->scale_),
                    torch::kCUDA
                ).transpose(0, 1);

                if (!new_kf->set_projection_matrix_) {
                    new_kf->projection_matrix_ = new_kf->getProjectionMatrix(
                        new_kf->znear_,
                        new_kf->zfar_,
                        new_kf->FoVx_,
                        new_kf->FoVy_,
                    ...
                    new_kf->set_projection_matrix_ = true;
                }

                new_kf->full_proj_transform_ = (new_kf->world_view_transform_.unsqueeze(0).bmm(
                    new_kf->projection_matrix_.unsqueeze(0))).squeeze(0);

                new_kf->camera_center_ = torch::empty({3}, torch::kCUDA);
                new_kf->camera_center_ = new_kf->world_view_transform_.inverse().index({3, torch::indexing::Slice(0, 3)});
                torch::cuda::synchronize();
                // if (new_kf->camera_center_.defined()) {
                //     // std::cout << "camera_center_ is defined and has shape: " << new_kf->camera_center_.sizes() << std::endl;
                // } else {
                //     std::cerr << "camera_center_ is not defined or has invalid shape." << std::endl;
                // }
                // std::cout << "camera_center_: ";
            }
            // new_kf->computeTransformTensors4Eval();
            // std::cout << "2.4 ";
            new_kf->imageid_ = idx;
            ...
            //compare pose //mono的位姿与gt维姿有很大区别
            // if (idx == vec_kfID.front())
            if (vec_kfID.size() > 0){
            if (Traj[idx][0] == vTimestamps[vec_kfID.front()])
            {
                ...
                // new_kf->setPose(kfPose.unit_quaternion().cast<double>(), kfPose.translation().cast<double>());//to test if low PSNR is due to pose diff
            ...
            //progress bar
            {
                ...
                std::cout << "] " << int(progress * 100.0) << " % " << "pKF->mnId: " << idx << " " << vTimestamps.size() << " " << vec_kfID.size() << "\r";
                std::cout.flush();
            }
